...
function Otp({ onUpdateParentState }) {
  ...
  return (
    <form
      className="w-full  my-4   mx-12   flex flex-col md:justify-center gap-4 md:items-start md:w-auto"
      ...
      <div className="flex mt-3 gap-4 justify-center items-center">
        {inputRefs.map((inputRef, index) => (
          <input
            key={index}
            ref={inputRef}
            className="w-full rounded-[6px] text-[30px] h-[39px] pl-[12px] focus:outline-none my-2 focus:bg-[#F6F6F6] bg-[#efeeee]  placeholder:text-[13px] placeholder:text-[#8B8989] md:w-[50px] md:h-[50px] md:placeholder:text-[14px] md:pl-[15px]"
            style={{
